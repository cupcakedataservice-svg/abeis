...
//   buildExportRecords,
//   recordsToCsv,
// } = require("../services/exportBaselineDataset");
// const {
...
const { getExportRows, rowsToCsv } = require("../services/exportService");
...
// GET /api/admin/export
//
// Produces one unified, baseline/ML-ready record per assessment by joining
// users, assessmentresponses, behavioralfeatures, media, and
// ExtractedBehaviorFeatures on assessmentId — see services/exportService.js
// for the join + shaping logic. `behavioralFeatures` is a real nested
// object now (previously a stringified `featureVectorJSON`), and the new
// `videoFeatures` block carries the AI-extracted webcam/screen features.
// Either is `null` (never omitted, never an error) if its source document
// doesn't exist or, for videoFeatures, hasn't finished processing yet.
const exportDataset = asyncHandler(async (req, res) => {
  const { format = "json", assessmentType } = req.query;

  const rows = await getExportRows({ assessmentType });
