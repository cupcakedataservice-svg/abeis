...
v2 additions (unchanged): averageFaceConfidence, continuousFaceLossCount,
maximumFaceLossDuration, averageBlinkDuration, maximumBlinkDuration,
blinkIntervalVariance, screenAttentionPercentage, averageLookAwayDuration,
maximumLookAwayDuration, pitchStdDeviation, yawStdDeviation,
rollStdDeviation, averageHeadSpeed, maximumHeadSpeed

v3 additions (unchanged field set): numberOfFaces, faceBoundingBox,
averageFaceSize, averageFacePosition, faceDisappearanceDuration,
lookingLeftDuration, lookingRightDuration, lookingUpDuration,
lookingDownDuration, lookingLeftPercentage, lookingRightPercentage,
lookingUpPercentage, lookingDownPercentage

--------------------------------------------------------------------------
AUDIT FIXES (this pass) — no fields added or removed, only formulas
corrected. See README "Corrected Feature Reference" for the full writeup.
--------------------------------------------------------------------------

1. Blink timing is now driven entirely by ear.py's real-time (ms) logic
   instead of a fixed frame count — see utils/ear.py's module docstring.
   `BlinkTracker` is now constructed WITH `seconds_per_sample` and
   `finalize()` no longer takes it as an argument.

2. Face bounding box / averageFaceSize / averageFacePosition / face
   confidence now come from the ONE Face Detection box that's matched (by
   centroid distance) to the face Face Mesh is actively tracking, instead
   of "whichever detection scored highest this frame." Previously, in a
   multi-person recording, the highest-confidence face could silently
   change from frame to frame (e.g. whichever person is better lit or
   closer that moment), so the averaged bounding box/position could blend
   together two different people's positions into a physically
   meaningless number. Matching to the tracked primary face keeps every
   frame's contribution describing the SAME person Face Mesh (and
   therefore EAR/blink/head-pose) is already analyzing.

3. `numberOfFaces` now applies a stricter confidence floor
   (SECOND_FACE_MIN_CONFIDENCE) than the base detector threshold, to cut
   down on false-"second person" reports from reflections, posters, or
   photos in the background. The underlying "run Face Detection on every
   sampled frame, independent of Face Mesh's single-face limit" structure
   — which is what actually lets this field report more than 1 in the
   first place — was already correct in the prior version and is
   unchanged here.

4. Face-confidence samples are only recorded when a matching detection is
   found for the tracked face (see #2). Previously a frame with no
   matching detection appended a hard `0.0`, which silently dragged the
   whole-recording average down even though nothing was actually measured
   in that frame.

5. Gaze-direction buckets (looking left/right/up/down) now require a
   small number of consecutive frames in the same direction
   (GAZE_MIN_CONSEC_FRAMES) before they start counting, matching the
   existing debounce already used for the look-away/attention signal.
   Previously every single frame counted independently, so one-frame
   landmark jitter could flip a gaze bucket on and off and inflate the
   percentages with noise.

All new/corrected logic still runs inside the SAME single MediaPipe pass
already being executed — no second video pass, no additional model
family, consistent with the "maintain approximately current processing
speed" requirement.
"""
...
# Separate, smaller thresholds used only for classifying WHICH of the four
# directions a frame's gaze falls into. Deliberately distinct from
# YAW_AWAY_DEG/PITCH_AWAY_DEG above: "looking away" is a coarse attention
# flag, while these four buckets are meant to catch a milder, more typical
# glance in a given direction.
GAZE_YAW_RIGHT_DEG = 12.0
GAZE_YAW_LEFT_DEG = -12.0
GAZE_PITCH_DOWN_DEG = 10.0
GAZE_PITCH_UP_DEG = -10.0
...
# AUDIT FIX #5: debounce for the four gaze-direction buckets, mirroring
# AWAY_CONSEC_FRAMES's purpose but intentionally smaller — these buckets
# are meant to catch shorter, more everyday glances than the coarse
# "looking away from the screen" signal above.
GAZE_MIN_CONSEC_FRAMES = 2

# AUDIT FIX #3/#4: confidence floor for (a) trusting a detection enough to
# use it for the primary face's confidence/bbox stats, and (b) counting a
# detection toward numberOfFaces. Intentionally higher than the detector's
# own internal min_detection_confidence=0.5 — that 0.5 just decides
# whether MediaPipe returns a candidate at all; this is a second, stricter
# filter applied to what we're willing to average into a stability metric
# or count as evidence of a second person.
MIN_CONFIDENCE_FOR_STATS = 0.6
SECOND_FACE_MIN_CONFIDENCE = 0.6
...
def _bbox_from_landmarks(landmarks_px, frame_w, frame_h):
    """Bounding box of the Face Mesh landmarks currently being tracked,
    normalized to [0, 1]. Used only to find which Face Detection box
    belongs to the SAME face — never returned directly (Face Detection's
    own box is what's reported, since it's a tighter, purpose-built face
    box rather than a mesh-landmark envelope)."""
    ...
    return {
        "x": xmin / frame_w,
        "y": ymin / frame_h,
        "width": (xmax - xmin) / frame_w,
        "height": (ymax - ymin) / frame_h,
    }


def _match_primary_detection(detections, landmark_bbox):
    """Pick the Face Detection box whose center is closest to the Face
    Mesh landmark box's center, i.e. the SAME face Face Mesh is tracking
    — rather than whichever detection scores highest this frame. This is
    what keeps faceBoundingBox / averageFaceSize / averageFacePosition /
    averageFaceConfidence describing one consistent identity across the
    whole recording instead of silently jumping between people whenever a
    second face happens to be more confidently detected for a frame or
    two."""
    if not detections:
        return None
    lm_cx = landmark_bbox["x"] + landmark_bbox["width"] / 2
    lm_cy = landmark_bbox["y"] + landmark_bbox["height"] / 2
    ...
    for det in detections:
        ...
        dist = (cx - lm_cx) ** 2 + (cy - lm_cy) ** 2
        if best_dist is None or dist < best_dist:
            best, best_dist = det, dist
    return best


class _AxisGazeCounter:
    """Counts frames spent in each of two opposite directions on one axis
    (e.g. left/right), requiring GAZE_MIN_CONSEC_FRAMES consecutive frames
    in the same direction before those frames start counting. This avoids
    single-frame landmark jitter flipping the gaze bucket back and forth
    and inflating both directions' percentages simultaneously.

    Trade-off (documented, deliberate): the first (GAZE_MIN_CONSEC_FRAMES
    - 1) frames of every qualifying run are not counted, which very
    slightly UNDER-counts total duration in exchange for not
    OVER-counting single-frame noise. With GAZE_MIN_CONSEC_FRAMES=2 this
    under-count is at most one sampled frame per direction-change, which
    is negligible relative to the jitter it prevents.
    """

    def __init__(self, min_consec_frames):
        self.min_consec_frames = max(1, min_consec_frames)
        self._current = None  # "pos" | "neg" | None
        self._run_length = 0
        self.pos_frames = 0
        self.neg_frames = 0

    def update(self, direction):
        if direction == self._current and direction is not None:
            self._run_length += 1
        else:
            self._current = direction
            self._run_length = 1 if direction is not None else 0

        if self._current is not None and self._run_length >= self.min_consec_frames:
...
def extract_webcam_features(video_path):
    ...
    # Effective real-world seconds represented by each *processed* frame,
    # since sampling may skip raw frames on long videos.
    seconds_per_sample = (total_frames / max(len(sample_indices_list), 1)) / fps if fps > 0 else 0

    # AUDIT FIX #1: seconds_per_sample now supplied at construction time so
    # every blink-timing decision is made in real time, not frame counts.
    blink_tracker = BlinkTracker(seconds_per_sample=seconds_per_sample)
    ...
    face_loss_run = 0
    face_loss_run_lengths = []
    face_loss_total_frames = 0  # cumulative count, independent of run length

    # Face framing accumulators (only over frames with a matched primary detection)
    bbox_xs, bbox_ys, bbox_ws, bbox_hs = [], [], [], []
    max_faces_seen = 0

    horizontal_gaze = _AxisGazeCounter(GAZE_MIN_CONSEC_FRAMES)  # pos=right, neg=left
    vertical_gaze = _AxisGazeCounter(GAZE_MIN_CONSEC_FRAMES)    # pos=down,  neg=up
    ...
    with mp_face_mesh.FaceMesh(
        ...
        while True:
            ...
            # Face Detection runs on EVERY sampled frame (independent of
            # Face Mesh's max_num_faces=1 limit) so numberOfFaces reflects
            # the true simultaneous face count, including a possible
            # second person, even in a frame where Face Mesh's single
            # tracked face is momentarily lost. Still one call per
            # already-processed frame, not a second video pass.
            detection_result = face_detector.process(rgb)
            detections = detection_result.detections or []

            # AUDIT FIX #3: stricter confidence floor for counting a face
            # toward numberOfFaces, to reduce false positives.
            num_faces_this_frame = sum(1 for d in detections if d.score[0] >= SECOND_FACE_MIN_CONFIDENCE)
            max_faces_seen = max(max_faces_seen, num_faces_this_frame)
            ...
            # AUDIT FIX #2/#4: match the Face Detection box to the SAME
            # face Face Mesh is tracking (by centroid distance) instead of
            # taking whichever detection scored highest this frame. Only
            # record a confidence/bbox sample when a sufficiently
            # confident match is found — a frame with no confident match
            # contributes nothing, rather than a misleading 0.0.
            landmark_bbox = _bbox_from_landmarks(landmarks_px, frame_w, frame_h)
            primary_detection = _match_primary_detection(detections, landmark_bbox)
            if primary_detection is not None and primary_detection.score[0] >= MIN_CONFIDENCE_FOR_STATS:
                face_confidences.append(primary_detection.score[0])
                box = primary_detection.location_data.relative_bounding_box
                bbox_xs.append(box.xmin)
                bbox_ys.append(box.ymin)
                bbox_ws.append(box.width)
                bbox_hs.append(box.height)
            ...
            if pose:
                ...
                # AUDIT FIX #5: debounced four-directional gaze bucketing,
                # reusing this same pose estimate.
                horiz_direction = "pos" if yaw > GAZE_YAW_RIGHT_DEG else ("neg" if yaw < GAZE_YAW_LEFT_DEG else None)
                vert_direction = "pos" if pitch > GAZE_PITCH_DOWN_DEG else ("neg" if pitch < GAZE_PITCH_UP_DEG else None)
                horizontal_gaze.update(horiz_direction)
                vertical_gaze.update(vert_direction)
    ...
    # AUDIT FIX #1: finalize() no longer takes seconds_per_sample.
    blink_stats = blink_tracker.finalize()
    blink_diagnostics = blink_stats.pop("_diagnostics", {})
    ...
    # --- look-away duration stats (frames -> seconds) ---
    away_durations_s = [n * seconds_per_sample for n in away_run_lengths]
    average_look_away_duration = float(np.mean(away_durations_s)) if away_durations_s else 0.0
    maximum_look_away_duration = float(np.max(away_durations_s)) if away_durations_s else 0.0

    # --- face-loss duration stats (longest single run) ---
    face_loss_durations_s = [n * seconds_per_sample for n in face_loss_run_lengths]
    maximum_face_loss_duration = float(np.max(face_loss_durations_s)) if face_loss_durations_s else 0.0

    # --- face-loss duration, CUMULATIVE total across the whole recording ---
    face_disappearance_duration = round(face_loss_total_frames * seconds_per_sample, 2)

    # --- head speed (deg/sec) between consecutive frames that both had a valid pose ---
    head_speeds = []
    for (pos_a, pa, ya, ra), (pos_b, pb, yb, rb) in zip(pose_history, pose_history[1:]):
        dt = (pos_b - pos_a) * seconds_per_sample
        if dt <= 0:
    ...
    # --- face framing (average bounding box / size / position) ---
    face_bounding_box = None
    average_face_size = 0.0
    average_face_position = None
    if bbox_xs:
    ...
    # --- gaze-direction durations + percentages ---
    looking_right_frames = horizontal_gaze.pos_frames
    looking_left_frames = horizontal_gaze.neg_frames
    looking_down_frames = vertical_gaze.pos_frames
    looking_up_frames = vertical_gaze.neg_frames
