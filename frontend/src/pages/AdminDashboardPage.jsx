...
export default function AdminDashboardPage() {
  ...
  const confirmDeleteOne = async () => {
    ...
    try {
      ...
      setBannerType(status === 207 || data.failures?.length ? "warning" : "success");
      setSuccessMsg(data.message);
      setDeleteTarget(null);
      loadData();
    } catch (err) {
      setBannerType("warning");
      setSuccessMsg(err.response?.data?.message || "Deletion failed. Please try again.");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const confirmDeleteSelected = async () => {
    setDeleting(true);
    try {
      const { data } = await api.post("/admin/users/delete-selected", { userIds: Array.from(selected) });
      setBannerType(data.skipped?.length || data.failures?.length ? "warning" : "success");
      setSuccessMsg(data.message);
  ...
  const confirmClearAll = async () => {
    ...
    setDeleting(true);
    try {
      const { data, status } = await api.post("/admin/clear-all", { confirmation: "DELETE ALL DATA" });
      setBannerType(status === 207 || data.failures?.length ? "warning" : "success");
      setSuccessMsg(data.message);
      ...
      setBannerType("warning");
      setSuccessMsg(err.response?.data?.message || "Failed to clear dataset. Please try again.");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="min-h-screen px-4 py-8 max-w-7xl mx-auto">
      ...
      {successMsg && (
        ...
        >
          ...
          <button
            ...
            className={bannerType === "warning" ? "ml-4 text-amber-500 hover:text-amber-200" : "ml-4 text-green-600 hover:text-green-300"}
          >
            ✕
          </button>
        </div>
      ...
      {overview && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <StatCard label="Total Assessments" value={overview.assessmentCount} />
          <StatCard label="Completed" value={overview.completedCount} />
          <StatCard
            label="By Type"
            value={overview.byType.map((b) => `${b._id}: ${b.count}`).join(" / ") || "—"}
            small
          />
        </div>
      )}
      ...
      <form onSubmit={handleFilter} className="card p-4 mb-6">
        <div className="flex flex-wrap gap-3">
          <input className="input-field max-w-xs" placeholder="Search name, email, or User ID" value={search} onChange={(e) => setSearch(e.target.value)} />
          <select className="input-field max-w-[180px]" value={assessmentType} onChange={(e) => setAssessmentType(e.target.value)}>
            <option value="">All Types</option>
            <option value="mcq">MCQ</option>
            <option value="coding">Coding</option>
            <option value="typing">Typing</option>
          </select>
          <select className="input-field max-w-[180px]" value={calibration} onChange={(e) => setCalibration(e.target.value)}>
            <option value="">Calibration (any)</option>
            <option value="yes">Calibration: Yes</option>
            <option value="no">Calibration: No</option>
          </select>
          <select className="input-field max-w-[190px]" value={finalAssessment} onChange={(e) => setFinalAssessment(e.target.value)}>
            <option value="">Final Assessment (any)</option>
            <option value="yes">Final: Yes</option>
            <option value="no">Final: No</option>
          </select>
          <input type="date" className="input-field max-w-[160px]" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} title="Date from" />
          <input type="date" className="input-field max-w-[160px]" value={dateTo} onChange={(e) => setDateTo(e.target.value)} title="Date to" />
      ...
      {selected.size > 0 && (
        <div className="mb-4 flex items-center gap-3 bg-accent/10 border border-accent/20 rounded-lg px-4 py-2">
          <span className="text-sm text-accent">{selected.size} user{selected.size > 1 ? "s" : ""} selected</span>
          <button
      ...
      {loading ? (
        ...
        <div className="card overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {users.map((row) => (
                <tr key={row.user.userId} className="border-b border-white/5 hover:bg-white/5">
                  <td className="p-3">
                    <input
                      type="checkbox"
                  ...
                  <td className="p-3 font-mono text-xs text-white/40">{row.user.userId}</td>
                  <td className="p-3">{row.user.name}</td>
                  <td className="p-3 text-white/60">{row.user.email}</td>
                  <td className="p-3 text-white/40 text-xs">{new Date(row.user.createdAt).toLocaleDateString()}</td>
                  <td className="p-3">{row.assessmentCount}</td>
                  <td className="p-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${row.hasCalibration ? "bg-green-900/40 text-green-400" : "bg-white/5 text-white/30"}`}>
                      {row.hasCalibration ? "Yes" : "No"}
                    </span>
                  </td>
                  <td className="p-3">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${row.hasFinal ? "bg-green-900/40 text-green-400" : "bg-white/5 text-white/30"}`}>
                      {row.hasFinal ? "Yes" : "No"}
                    </span>
                  </td>
                  <td className="p-3 text-white/40 text-xs">
                    {row.lastAssessmentDate ? new Date(row.lastAssessmentDate).toLocaleDateString() : "—"}
                  </td>
                  <td className="p-3">
                    <div className="flex gap-2">
                      <button onClick={() => navigate(`/admin/users/${row.user.userId}`)} className="text-accent hover:underline text-xs">
                        View
                      </button>
                      <button
          ...
          {users.length === 0 && <p className="p-6 text-center text-white/40">No users found.</p>}
        </div>
      )}

      {/* ── Delete single dialog ─── */}
      {deleteTarget && (
        <Modal title="Delete Participant Data" onClose={() => setDeleteTarget(null)}>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button onClick={confirmDeleteOne} className="px-4 py-2 rounded-lg bg-red-700 hover:bg-red-600 text-white text-sm font-medium" disabled={deleting}>
      ...
      {showDeleteSelected && (
        <Modal title="Delete Selected Participants" onClose={() => setShowDeleteSelected(false)}>
          <p className="text-white/60 text-sm mb-4">
            You are about to permanently delete <span className="text-white font-semibold">{selected.size} participant(s)</span> and all their associated data.
          </p>
          <p className="text-red-400 text-sm mb-6">This action cannot be undone.</p>
          <div className="flex gap-3 justify-end">
            <button onClick={() => setShowDeleteSelected(false)} className="btn-secondary" disabled={deleting}>Cancel</button>
            <button onClick={confirmDeleteSelected} className="px-4 py-2 rounded-lg bg-red-700 hover:bg-red-600 text-white text-sm font-medium" disabled={deleting}>
      ...
      {/* ── Clear all dialog ─── */}
      {showClearAll && (
        <Modal title="Clear Entire Dataset" onClose={() => setShowClearAll(false)}>
          <p className="text-white/60 text-sm mb-4">
            This will permanently remove <strong className="text-white">every participant, assessment, recording, and behavioral record</strong> from the system.
          </p>
          <p className="text-sm text-white/60 mb-2">Type <code className="text-red-400">DELETE ALL DATA</code> to confirm:</p>
          <input
          ...
          />
          <div className="flex gap-3 justify-end">
            <button onClick={() => setShowClearAll(false)} className="btn-secondary" disabled={deleting}>Cancel</button>
            <button
...
function StatCard({ label, value, small }) {
  return (
    <div className="card p-4">
      ...
      <p className={small ? "text-sm" : "text-2xl font-display font-bold"}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
      <div className="card w-full max-w-md p-6 relative">
        ...
        {children}
        <button onClick={onClose} className="absolute top-4 right-4 text-white/30 hover:text-white/60 text-lg leading-none">✕</button>
      </div>
    </div>
  );
}
